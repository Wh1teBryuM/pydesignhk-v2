...
export default function TrackProject() {
  ...
  async function fetchProjects(token) {
    ...
    setError("")
    try {
      const res  = await fetch("https://pydesignhk-backend.onrender.com/tracker/customer/projects", { headers: { authorization: token } })
        ...
        {!loading && !error && projects.map((proj) => {
          ...
          return (
            <div key={proj.id} style={{ marginBottom: "16px", border: `1px solid ${BORDER_SUBTLE}` }}>
              ...
              <div
                style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 24px", cursor: "pointer", background: isExpanded ? "rgba(255,255,255,0.02)" : "transparent" }}
                onClick={() => setExpandedId(isExpanded ? null : proj.id)}
              >
                <div>
                  <h2 style={{ fontFamily: "Georgia, serif", fontWeight: 400, fontSize: "20px", color: "#fff", margin: "0 0 4px" }}>{proj.project_name}</h2>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
                  ...
                  <span style={{ color: TEXT_MUTED, fontSize: "12px" }}>{isExpanded ? "▲" : "▼"}</span>
                </div>
              </div>

              {/* Expanded content */}
              {isExpanded && (
                <div style={{ padding: "0 24px 32px", borderTop: `1px solid ${BORDER_SUBTLE}` }}>
                  ...
                  {phases.map((phase, idx) => {
                    ...
                    return (
                      <div key={phase.id} style={{ display: "flex", gap: "20px" }}>
